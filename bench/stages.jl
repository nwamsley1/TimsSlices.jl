...
buf = FrameBuffer(); sc = SmoothScratch(); out = FrameSlices(); blk = SliceBlock(); codec = BlockCodec(); sb = SliceBuffer()
...
function stage_times(i)
    ...
    nb = 0; payloads = Vector{UInt8}[]
    for j in 1:blk.n_slices; b = encode_slice!(codec, blk, j, p.zstd_level); nb += b; push!(payloads, codec.zbuf[1:b]); end
    t4 = time_ns()
    for j in 1:blk.n_slices; decode_slice!(sb, codec, payloads[j], length(TS.slice_range(blk, j))); end
    t5 = time_ns()
