...
Base.@kwdef struct ConvertParams
    ...
    mz_sigma_alpha::Float64 = 0.0    # != 0: sigma_bins = mz_sigma (mz / mz_sigma_ref)^alpha (0.5 = constant ppm, -0.5 = constant mDa)
    mz_sigma_ref::Float64 = 600.0
...
function validate(p::ConvertParams)
    ...
    p.mz_sigma_ref > 0 || throw(ArgumentError("mz_sigma_ref must be > 0"))
...
function output_name(source::AbstractString, p::ConvertParams)
    ...
    mz = fmt(p.mz_sigma) * (p.mz_sigma_alpha != 0 ? "a$(fmt(p.mz_sigma_alpha))" : "")
