...
function convert(dir::AbstractString, out_dir::AbstractString; params::ConvertParams = ConvertParams(),
    ...
    ls1 = LevelSetup(level_params(p, true), p.mz_sigma_alpha, p.mz_sigma_ref, f.mz_cal, n_bins(f))
    ls2 = LevelSetup(level_params(p, false), p.mz_sigma_alpha, p.mz_sigma_ref, f.mz_cal, n_bins(f))
