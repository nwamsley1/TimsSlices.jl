...
Options (defaults in brackets):
  ...
  --mz-sigma S [3]  --mz-sigma-alpha A [0]  --mz-sigma-ref M [600]  sigma = mz-sigma (mz/M)^A bins  --centroid wmean|gauss|none [wmean]  --max-half H [max(4, 4*mz-sigma)]
  --min-scans N [1]
...
function parse_cli(args::Vector{String})
    ...
    flags = Dict("--stride-k0" => (:stride_k0, Float64), "--im-sigma-k0" => (:im_sigma_k0, Float64),
                 ...
                 "--stride" => (:stride, Int), "--ms1-stride" => (:ms1_stride, Int), "--mz-sigma" => (:mz_sigma, Float64), "--mz-sigma-alpha" => (:mz_sigma_alpha, Float64), "--mz-sigma-ref" => (:mz_sigma_ref, Float64),
